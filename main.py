...
QUIZ_QUESTIONS = [
    ...
    QuizQuestion(
        "Что делать при подозрительном звонке от якобы сотрудника банка?",
...
# Reads a .env file and sets missing environment variables in the current process.
...
# Returns bot token from environment and raises actionable error if missing.
def get_bot_token() -> str:
    ...
    raise RuntimeError(
        "BOT_TOKEN не найден. Создайте файл .env рядом с main.py и добавьте строку "
        "BOT_TOKEN=<ваш_токен> или задайте переменную окружения вручную."
    )
